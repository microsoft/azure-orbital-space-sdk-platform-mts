...
public partial class PluginDelegates {
    internal MessageFormats.HostServices.Sensor.TaskingPreCheckResponse? TaskingPreCheckResponse((MessageFormats.HostServices.Sensor.TaskingPreCheckResponse? input_response, Plugins.PluginBase plugin) input) {
        const string methodName = nameof(input.plugin.SensorsAvailableRequest);

        if (input.input_response is null || input.input_response is default(MessageFormats.HostServices.Sensor.TaskingPreCheckResponse)) {
        ...
        try {
            Task<MessageFormats.HostServices.Sensor.TaskingPreCheckResponse?> pluginTask = input.plugin.TaskingPreCheckResponse(input_response: input.input_response);
            pluginTask.Wait();
